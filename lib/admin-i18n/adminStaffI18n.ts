...
export function adminStaffPage(locale: AdminLocale) {
  return {
    ...
    deleteConfirmTitle: pick(locale, 'Удалить сотрудника?', '删除该员工？'),
    deleteConfirmMessage: pick(
      locale,
      'Учётная запись будет безвозвратно удалена из списка. История действий сохранится в журнале.',
      '该账号将从列表中永久删除，操作历史仍保留在日志中。',
    ),
    deleteSuccess: pick(locale, 'Сотрудник удалён', '员工已删除'),
    errDelete: pick(locale, 'Не удалось удалить сотрудника', '删除失败'),

...
/**
 * Прокси загрузки в чат заказа: пересылаем **исходный** поток multipart
 * (boundary из заголовка), без буфера `arrayBuffer()` / пересборки FormData —
 * иначе Multer на Nest иногда получает пустое тело.
 */
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const path = `orders/${params.id}/chat/upload`.replace(/^\//, '');
  ...
  try {
    ...
    console.error('[chat/upload proxy] fetch failed', url, e);

...
import {
  fetchCatalogFilterOptionsClient,
  fetchCatalogProductsSearchClient,
} from '@/lib/catalogProductsSearchClient';
import type { CatalogProductSearchHit, PublicCatalogTag } from '@/lib/catalogPublic';
...
type Props = {
  ...
  initialFilterOptions?: CatalogFilterOptions;
  zones: PublicCatalogTag[];
  initialProductCategoryId: string;
...
function patchCatalogSearchParams(patch: Record<string, string | null>) {
  if (typeof window === 'undefined') return;
  const url = new URL(window.location.href);
...
function writeTagSlugsToHistory(slugs: string[]) {
  patchCatalogSearchParams({ tag: catalogTagsQuery(slugs) ?? null });
}
...
function writeFacetsToHistory(facets: CatalogFacetFilters) {
  patchCatalogSearchParams(catalogFacetFiltersToPatch(facets));
}

function writeSortToHistory(sort: CatalogSortId) {
  patchCatalogSearchParams({ sort: sort === 'popular' ? null : sort });
}
...
export function CatalogCategoryMarketClient({
  initialTagSlug,
  initialPriceFrom,
  initialPriceTo,
  initialFacets,
  ...
  const { productCategoryId } = useCatalogBrowse();
  ...
  const [filterOptions, setFilterOptions] = useState<CatalogFilterOptions>(
    () => initialFilterOptions ?? { ...EMPTY_CATALOG_FILTER_OPTIONS },
  );
  ...
  const tagSlugParam = useMemo(() => catalogTagsQuery(selectedZoneSlugs), [selectedZoneSlugs]);
  const initialTagParam = useMemo(
    () => catalogTagsQuery(parseCatalogTagSlugs(initialTagSlug)) ?? '',
    [initialTagSlug],
  );
  const initialPriceKey = `${initialPriceFrom ?? ''}:${initialPriceTo ?? ''}`;
  const priceKey = `${priceRange.priceFrom ?? ''}:${priceRange.priceTo ?? ''}`;
  const facetsKey = catalogFacetFiltersKey(facets);
  const initialFacetsKey = catalogFacetFiltersKey(initialFacets ?? EMPTY_CATALOG_FACET_FILTERS);
  ...
  const commitPriceRange = useCallback((next: PriceRange) => {
    const normalized = normalizeCatalogPriceRange(next.priceFrom, next.priceTo);
    setPriceRange(normalized);
    setDraftFrom(priceBoundToInputValue(normalized.priceFrom));
    setDraftTo(priceBoundToInputValue(normalized.priceTo));
    writePriceToHistory(normalized);
  }, []);
  ...
  const loadProducts = useCallback(
    async (
      ...
      price: PriceRange,
      nextFacets: CatalogFacetFilters,
    ) => {
      ...
      try {
        ...
        if (gen === loadGenRef.current) setResultsPending(false);
      }
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    const isInitialTag = (tagSlugParam ?? '') === initialTagParam;
    const isInitialSort = sortId === initialSort;
    const isInitialPrice = priceKey === initialPriceKey;
    const isInitialFacets = facetsKey === initialFacetsKey;

    if (isInitialCategory && isInitialTag && isInitialSort && isInitialPrice && isInitialFacets) {
      setHits(initialHits);
      setTotal(initialTotal);
      setResultsPending(false);
      return;
    }
    void loadProducts(productCategoryId, sortId, tagSlugParam, priceRange, facets);
  }, [
    productCategoryId,
    initialProductCategoryId,
    initialHits,
  ...
  useEffect(() => {
    const needsNames = facets.brandIds.length > 0 || facets.materialIds.length > 0;
    if (!filtersOpen && !needsNames && !hasActiveCatalogFacetFilters(facets) && !tagSlugParam) {
      return;
    }
    let cancelled = false;
    void fetchCatalogFilterOptionsClient({
      ...
      const brandSet = new Set(opts.brands.map((b) => b.id));
      const materialSet = new Set(opts.materials.map((m) => m.id));
      setFacets((prev) => {
        const brandIds = prev.brandIds.filter((id) => brandSet.has(id));
        const materialIds = prev.materialIds.filter((id) => materialSet.has(id));
        if (
          brandIds.length === prev.brandIds.length &&
          materialIds.length === prev.materialIds.length
        ) {
          return prev;
        }
        const next = { ...prev, brandIds, materialIds };
        writeFacetsToHistory(next);
        return next;
      });
    });
    return () => {
  ...
  useEffect(() => {
    ...
    prevProductCategoryIdRef.current = productCategoryId;
    setFacets((current) => {
      if (!current.brandIds.length && !current.materialIds.length) return current;
      const next = { ...current, brandIds: [] as string[], materialIds: [] as string[] };
      writeFacetsToHistory(next);
      return next;
    });
  }, [productCategoryId]);
    ...
    <>
        ...
        <div className={styles.marketToolbar}>
          ...
          <div className={styles.marketToolbarEnd}>
            <p className={styles.marketProductCount} aria-live="polite">
              {resultsPending ? '[…]' : formatCatalogProductCount(total)}
            </p>
      ...
      >
        ...
        <div
          className={
            resultsPending
              ? `${styles.marketProductsSlot} ${styles.marketProductsSlotPending}`
              : styles.marketProductsSlot
          }
          aria-busy={resultsPending}
        >
          {resultsPending ? (
            <p className={styles.marketResultsStatus} role="status">
              Загрузка…
            </p>
          ) : null}
          <CategoryCatalogGridClient
            ...
            priceTo={priceRange.priceTo}
            facets={facets}
            columns={filtersOpen ? 3 : 4}
            initialHits={hits}
            initialTotal={total}
            canResetFilters={hasFilterChips}

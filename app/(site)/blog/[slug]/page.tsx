...
import {
  blogArticleMetadataFromPost,
  blogArticleNotFoundMetadata,
} from '@/lib/blog/mapBlogMetadata';
...
import projectsStyles from '@/app/(site)/(public)/projects/ProjectsPage.module.css';
...
export async function generateMetadata({
  ...
  if (!slug) return blogArticleNotFoundMetadata();
  const post = await loadBlogPostBySlug(slug);
  if (!post) return blogArticleNotFoundMetadata();
  const siteOrigin = await getServerRequestOrigin();
  return blogArticleMetadataFromPost(post, { siteOrigin });
}
    ...
    <main>
      <section className={articleStyles.articleMainSection} aria-label="Статья">
        <div className="padding-global">
          ...
          <div
            className={`rich-content ${articleStyles.articleBody}`}
